      ...
      {rows.map((r) => (
        <li key={r.name}>
          <button
            ...
            className="group w-full rounded-md px-1.5 py-1 text-left transition hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
            title={`Ask about ${r.name}`}
          >
            <div className="flex items-baseline justify-between gap-2 text-[11px]">
              <span className="truncate text-slate-700 group-hover:text-emerald-700 dark:text-slate-200 dark:group-hover:text-emerald-300">
                {r.name}
              </span>
            ...
            <div className="mt-0.5 h-1 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
              <div className="h-full rounded-full bg-emerald-500/70" style={{ width: `${(r.n / max) * 100}%` }} />
            </div>
          </button>
        </li>
      ))}
  ...
  return (
    <aside className="hidden w-64 shrink-0 overflow-y-auto border-r border-slate-200 bg-white px-4 py-5 dark:border-slate-700 dark:bg-slate-800 lg:block">
      <div className="space-y-5">
        <Section title="Recall trend">
          <button
            onClick={() => onAsk("How have RV recalls trended over the last 10 years?")}
            className="w-full rounded-lg p-1 transition hover:bg-slate-50 dark:hover:bg-slate-700/40"
            title="Ask about the recall trend"
          >
            <Sparkline points={data?.trend ?? []} />
          </button>
        </Section>

        <Section title="Top defect components">
          <MiniList
            rows={(data?.components ?? []).slice(0, 8).map((c) => ({ name: c.component, n: c.n }))}
            onPick={(name) => onAsk(`Which RV makes have the most ${name.toLowerCase()} complaints?`)}
          />
        </Section>

        <Section title="Top makes by recalls">
          <MiniList
            rows={(data?.makes ?? []).slice(0, 8).map((m) => ({ name: m.make_canonical, n: m.recalls }))}
